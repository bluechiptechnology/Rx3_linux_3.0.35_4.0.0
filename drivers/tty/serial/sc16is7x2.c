...
static int sc16is7x2_startup(struct uart_port *port)
{
	 ...
	 * one shot behaviour */
	if (request_irq(ts->spi->irq, sc16is7x2_irq, IRQF_TRIGGER_FALLING | IRQF_SHARED, "sc16is7x2", chan)) 
	{
...
static int __devinit sc16is7x2_probe(struct spi_device *spi)
{
	...
	//Temp = sc16is7x2_read(ts, UART_SCR, 0);
	//printk(KERN_ERR "SCR %x\n", Temp);
	///sc16is7x2_write(ts, UART_SCR, 0, 0xAA);
	//Temp = sc16is7x2_read(ts, UART_SCR, 0);
	//printk(KERN_ERR "SCR %x\n", Temp);
